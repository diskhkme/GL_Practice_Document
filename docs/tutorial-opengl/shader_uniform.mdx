...
정점 셰이더의 입력으로 전달할 수 있는 데이터는 크게 두 종류가 있는데, 한 가지가 이전에 알아본 [정점 어트리뷰트](./shader_and_vertex_attribute.md)이고, 다른 한 가지가 이번 시간에 알아볼 유니폼입니다.
...
잘 기억이 나지 않는다면 [어트리뷰트 개념도](shader_and_vertex_attribute.md#정점-어트리뷰트)를 참고하세요.
...
![Result1](assets/shader_uniform/img/result1.png)

계획한 대로 사각형이 오른쪽으로 잘 이동해 있네요.

지금 상황을 개념적으로 그림으로 표현해 보면 아래와 같습니다. 유니폼은 해당 셰이더 프로그램 전체가 동일한 값을 사용한다는 것이 핵심입니다.

![Uniform Diagram](assets/shader_uniform/img/uniform_diagram.png)
...
![Result2](assets/shader_uniform/img/result2.gif)
...
## 관련 링크
- [main.cpp 소스 코드](assets/shader_uniform/src/main_end.cpp)
- [basic.shader 소스 코드](assets/shader_uniform/src/basic.shader)
